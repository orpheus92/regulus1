...
class CycleCanvasExtension implements ICycleCanvasExtension {
    ...
    addCanvasButton(panel: NotebookPanel, context: DocumentRegistry.IContext<INotebookModel>) {
        ...
        let callback = () => { this.showCanvas(panel,context); }
    ...
    showCanvas(panel: NotebookPanel, context: DocumentRegistry.IContext<INotebookModel>) {
        let canvas = this._canvas = this._canvas || new CycleCanvas(panel,context);
